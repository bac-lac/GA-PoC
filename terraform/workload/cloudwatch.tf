...
resource "aws_cloudwatch_metric_alarm" "ga_cw_nlb_22_alarm" {
  ...
  statistic                 = "Minimum"
  dimensions                = zipmap(["TargetGroup", "LoadBalancer"], [aws_lb_target_group.ga_tg_22.name, "${var.NLB_NAME}"])
  period                    = 60
  evaluation_periods        = 5
  datapoints_to_alarm       = 5
  threshold                 = 1
...
resource "aws_cloudwatch_metric_alarm" "ga_cw_alb_443_alarm" {
  ...
  statistic                 = "Minimum"
  dimensions                = zipmap(["TargetGroup", "LoadBalancer"], [aws_lb_target_group.ga_tg_443.name, "${var.ALB_NAME}"])
  period                    = 60
  evaluation_periods        = 5
  datapoints_to_alarm       = 5
  threshold                 = 1
...
resource "aws_cloudwatch_metric_alarm" "ga_cw_alb_8443_alarm" {
  ...
  namespace                 = "AWS/ApplicationELB"
  statistic                 = "Minimum"
  dimensions                = zipmap(["TargetGroup", "LoadBalancer"], [aws_lb_target_group.ga_tg_8443.name, "${var.ALB_NAME}"])
  period                    = 60
  evaluation_periods        = 5
  datapoints_to_alarm       = 5
  threshold                 = 1
